...
We believe that the Model Abstraction Framework accomplishes each of these goals.  The MAF is extremely flexible allowing it to model a large variety of systems at varying and alternating degrees of abstraction which is ideal for new designs.  It has the ability to detect critical failures.  It has the ability to examine multiple paths through the model and to randomly perturb the model in different ways to explore these paths.  It also generates human workload metrics which are valuable for setting safety thresholds and analyzing new designs.

Other researchers are currently pursuing the creation of metric gathering Actors which could be attached to a third party model to take specific performance measurements.  This behavior is particulary attractive when attempting to standardize model behavior.
...
An Unmanned Aerial System (UAS) plans to operate within the National Airspace System.  , see figures~\ref{fig:uasnasdirg} and ~\ref{fig:uasnasditg}.  The UAS is composed of a server (UAS Server), an Unmanned Aerial Vehicle Operator (UAV Operator), and an Unmanned Aerial Vehicle (UAV).  The UAV Operator controls the UAS Server through a graphical user interface (UAS GUI) which in turn controls the UAV.  The UAV will take off and land at an airport serving both manned and unmanned aerial vehicles.  The UAV Operator is located at this airport and visually monitors the takeoff and landing of the UAV.  UAV state is continuously shown on the UAS GUI.
...
A DiRG for this model showing the states and data flow for each Actor can be seen in figure~\ref{fig:uasnasdirg}.  A matching DiTG for the model showing the Actor communication channels can be seen in figure~\ref{fig:uasnasditg}.
...
\begin{itemize}
  \item The UAV has an unlimited flight time, never loses contact with the UAS Server, can takeoff and land without incident, and has accurate GPS data, is non line-of-sight.
  \item The UAS Server/GUI never loses connection to the FAA Server, always has instant communication with the UAV and FAA Server, has no bugs, can create flight plans, can detect NOTAMs on the flight plan, can automatically direct the UAV out of an emergency NOTAM, displays radar information from the UAV, never goes down.
  \item The UAV Operator detects all warnings displayed on the UAS GUI, generates flight plans that do not touch NOTAMS, can always deconflict the UAV, never gets fatigued.
  \item The FAA Server/GUI distributes NOTAMS and automatically detects if a flight plan needs to be approved by the ATC.
  \item The ATC detects all information displayed on the FAA GUI, can add NOTAMS with the FAA GUI, always adds NOTAMS correctly, approves all flight plans.
\end{itemize}
...
On the other hand the decision workload typically jumps all over the place.  While we believe the increase in decision workload does in fact increase workload we are unsure how to weight this value in comparison with the resource and temporal workload values.  This is something which needs to be verified through sensitivity and user studies in future work.

The last analysis we would like to make is the comparison of our workload metric to that of the adapted Wickens model from chapter~\ref{ch:metrics}.  The general flow of the two metrics is very similar, this is expected since everything in the Wickens model also exists in our workload metric.  What is more interesting is were the two metrics differ.  Our workload metric is comprised of 9 different values each of which can range from 0 to 2+.  This adds a fair amount of detail into the workload measurement which can be seen by comparing the two values for any of the different model variations.  Where Wickens model shows a flat line our metric shows a heart beat which tells us much more about what is causing the additional workload.
